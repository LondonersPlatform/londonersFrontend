...
export default function Header() {
  ...
  const { setRedirectPath, setLoginOpen, loginOpen } = useLoginModal();

  // Inside Header component
  useEffect(() => {
    const storedLoginOpen = sessionStorage.getItem("loginOpen");
    if (storedLoginOpen === "true") {
      setLoginOpen(true);
    }
  }, []);

  useEffect(() => {
    sessionStorage.setItem("loginOpen", loginOpen.toString());
  }, [loginOpen]);
    ...
    <>
      <header className=" mx-auto md:w-[85%] w-[90%]  flex items-center justify-between py-4 px-4 md:px-6">
        ...
        <div className="flex items-center  space-x-4">
          ...
          {(!isLoading && session) || (!isLoading && access_token) ? (
